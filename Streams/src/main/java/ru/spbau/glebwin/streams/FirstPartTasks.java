...
public final class FirstPartTasks {
    ...
    public static List<Album> sortedFavorites(Stream<Album> albums) {
        return albums.filter(a -> a.getTracks().stream().map(Track::getRating).anyMatch(r -> r > 95))
    ...
    // Сгруппировать альбомы по артистам (в качестве значения вместо объекта 'Album' использовать его название)
    ...
    public static Optional<Album> minMaxRating(Stream<Album> albums) {
        return albums.min(Comparator.comparingInt(
                a -> a.getTracks().stream().mapToInt(Track::getRating).max().orElse(0)
        ));
    }
    ...
    public static List<Album> sortByAverageRating(Stream<Album> albums) {
        return albums.sorted(Comparator.comparingDouble(
                (Album a) -> a.getTracks().stream().mapToInt(Track::getRating).average().orElse(0)
